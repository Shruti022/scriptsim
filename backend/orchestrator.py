...
def _build_pipeline(personas: list[str], skip_mapper: bool = False, skip_login: bool = False, run_id: str = "") -> SequentialAgent:
    ...
    return SequentialAgent(
        name=f"scriptsim_pipeline{suffix}",
...
async def run_scan(
    ...
    initial_state = {
        ...
        "skip_login": storage_state is not None or bool(cookies),
    ...
    # If a captured session is provided, inject it so personas start authenticated
    if storage_state:
        from tools.browser import inject_storage_state as _inject_state
        await _inject_state(storage_state)
        _log(f"[scan:{scan_id}] Captured session injected — skipping login")

    # If cookies string provided, inject them
    # If cookies provided, inject them and skip login
    if cookies:
        parsed_cookies = []
        for pair in cookies.split(';'):
            ...
            if '=' in pair:
                ...
                parsed_cookies.append({'name': name.strip(), 'value': value.strip(), 'url': target_url})
        if parsed_cookies:
            from tools.browser import inject_cookies as _inject
            await _inject(parsed_cookies)
            _log(f"[scan:{scan_id}] Cookies injected — skipping login")
    ...
    # persona_durations tracked in memory only (session.update not supported in InMemorySessionService)
