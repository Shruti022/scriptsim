...
CRITICAL RULES:
- ONLY report bugs that are explicitly listed in the persona reports above.
- Do NOT invent, infer, or hallucinate bugs that are not in the reports.
- If all persona reports have empty bugs arrays, output an empty bugs list.
- Do NOT assume a vulnerability exists just because a payload was accepted — only report it if the persona explicitly confirmed the exploit worked.
